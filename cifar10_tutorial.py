...
class Net(nn.Module):
    ...
    def forward(self, x):  # x.size = [4, 3, 32, 32] = [BATCH_SIZE, inC, inH, inW]
        x = self.conv1(x)
        x = self.conv2(x)
        # x = x.view(-1, 16 * 5 * 5)  # -1,指适应第2个参数(维度)的任意数量
        x = x.view(BATCH_SIZE, -1)  # 等同上行
        x = self.fc1(x)
        x = self.fc2(x)
        x = self.fc3(x)
        return x
...
# outputs = net(images)  # 每输入一张图片，网络输出一个10维向量
...
# _, predicted = torch.max(outputs, 1)  # 取10维向量中最大值的序列号
...
#
# accuracy = 'Epoch:{} | Accuracy of the network on the 10000 test images: {:.2f}%'.format(_, (100 * correct / total))
# write_in_log(accuracy)
# print(accuracy)
...
#         c = (predicted == labels).squeeze()  # .squeeze() 从tensor中删除所有大小是1的维度
#         for i in range(4):  # ???
#             label = labels[i]
